...
class VamPvtStreamerNew(Node):
    ...
    def _stream_pvt(self, target):
        ...
        # Without this, velocity clamping creates permanent position drift
        # that feeds back into larger errors (positive feedback loop).
        # Applied as a velocity bias (not position jump) to keep the PVT
        # chain smooth — position discontinuities cause cubic spline
        # artifacts in the firmware.
        #
        # When drift is large (near firmware safety threshold), the
        # correction dominates — velocity is redirected to reduce drift
        # rather than chase the target. This prevents safety stops without
        # disruptive PVT exit/re-enter.
        drift = prev - current
        drift_mag = np.max(np.abs(drift))

        # Nonlinear gain: gentle at small drift, aggressive near limit
        # At drift=0: effective_gain = drift_gain (e.g. 0.3)
        # At drift=max_drift: effective_gain ≈ 3x drift_gain
        ...
        # Desired velocity toward target (from last sent position)
        ...
        # Position = previous + velocity * dt (smooth chain, no jumps)
