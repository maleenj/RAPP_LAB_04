...
//   2. Put it on a GameObject. A VamData component is added automatically.
//   3. On that VamData, set `channel` in the Inspector (see VamData.cs for the list).
...
//   * data.Tensor("name")    -> a VamTensor for activation channels
// Joint values are in radians.

using UnityEngine;

[RequireComponent(typeof(VamData))]
public class VamVisualizerTemplate : MonoBehaviour
{
    VamData data;

    void Awake()
    {
        data = GetComponent<VamData>();
        // Prefer the event if you only want to react when fresh data arrives:
        data.OnData += OnNewFrame;
    }

    void OnNewFrame(VamFrame frame)
    {
    ...
    void Update()
    {
        ...
        //   VamTensor energy = data.Tensor("encoder_out_norm"); // [10]
        //   if (energy != null) {
