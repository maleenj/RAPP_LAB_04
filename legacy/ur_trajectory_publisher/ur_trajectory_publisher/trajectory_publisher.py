...
class TrajectoryPublisher(Node):
    def __init__(self):
        ...
        self.joint_mapping = {
            ...
            1: {'index': 1, 'factor': 1.0, 'offset': 0.0}, # shoulder_lift
            2: {'index': 2, 'factor': 1.0, 'offset': 0.0},      # elbow
            3: {'index': 3, 'factor': 1.0, 'offset': 0.0}, # wrist_1
    ...
    async def execute_waypoints(self):
        ...
        try:
            first_waypoint = True  # Flag to track the first waypoint
            ...
            for i, waypoint in enumerate(self.waypoints):
                ...
                if dwell_time > 0:
                    ...
                    # Trigger space key press for the first waypoint
                    if first_waypoint:
                        keyboard = Controller()
                        keyboard.press(Key.space)
                        keyboard.release(Key.space)
                        first_waypoint = False  # Update flag
                        self.get_logger().info(f'SPACE PRESSED!')
